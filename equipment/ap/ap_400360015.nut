...
{
	appendage.sq_AddFunctionName("onStart", "onStart_appendage_400360015")
	appendage.sq_AddFunctionName("onEnd", "onEnd_appendage_400360015")
}

function onStart_appendage_400360015(appendage)
{
	if (!appendage) return;
	...
	local CriticalDamage = 220;
	local PhysicalAttackDamage = 220;
	local MagicalAttackDamage = 220;
	local change_appendage = appendage.sq_getChangeStatus("400360015");

	if (!change_appendage)
	{
		change_appendage = appendage.sq_AddChangeStatus("400360015", obj, obj, 0, CHANGE_STATUS_TYPE_ATTACK_SPEED, false, attackSpeed);
	}
	if (change_appendage)
	{
...
function onEnd_appendage_400360015(appendage)
{
	if (!appendage) return;
