...
function onStart_appendage_400360021(appendage)
{
	...
	sq_SetParameterOcularSpectrum(spectrumAppendage, 950, 100, true, sq_RGBA(255, 192, 203, 255), sq_RGBA(255, 192, 203, 0), 2, 2, 2);
	sq_SetParameterOcularSpectrum(spectrumAppendage, 950, 100, true, sq_RGBA(255, 192, 203, 255), sq_RGBA(255, 192, 203, 0), 2, 0, 1);
	...
	local cooltime = -0.05;
	local CriticalDamage = 380;
	local PhysicalAttackDamage = 380;
	local MagicalAttackDamage = 380;
	local change_appendage = appendage.sq_getChangeStatus("400360021");

	if (!change_appendage)
	{
		change_appendage = appendage.sq_AddChangeStatus("400360021", obj, obj, 0, CHANGE_STATUS_TYPE_ATTACK_SPEED, false, attackSpeed);
	}
	if (change_appendage)
	{
