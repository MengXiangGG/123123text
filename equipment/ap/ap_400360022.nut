...
{
	appendage.sq_AddFunctionName("onStart", "onStart_appendage_400360022")
	appendage.sq_AddFunctionName("onEnd", "onEnd_appendage_400360022")
}

function onStart_appendage_400360022(appendage)
{
	if (!appendage) return;
	...
	sq_SetParameterOcularSpectrum(spectrumAppendage, 1000, 100, true, sq_RGBA(0, 255, 255, 255), sq_RGBA(0, 255, 255, 0), 2, 2, 2);
	sq_SetParameterOcularSpectrum(spectrumAppendage, 1000, 100, true, sq_RGBA(0, 255, 255, 255), sq_RGBA(0, 255, 255, 0), 2, 0, 1);
	...
	local stuckresistance = 50;
	local stuck = -50;
	local cooltime = -0.1;
	local CriticalDamage = 400;
	local PhysicalAttackDamage = 400;
	local MagicalAttackDamage = 400;
	local change_appendage = appendage.sq_getChangeStatus("400360022");

	if (!change_appendage)
	{
		change_appendage = appendage.sq_AddChangeStatus("400360022", obj, obj, 0, CHANGE_STATUS_TYPE_ATTACK_SPEED, false, attackSpeed);
	}
	if (change_appendage)
	{
...
function onEnd_appendage_400360022(appendage)
{
	if (!appendage) return;
