...
function requestBuy(obj, skill, nIndex, flag, count)
{
	if(sq_getJob(obj) == ENUM_CHARACTERJOB_PRIEST && sq_getGrowType(obj) == GROW_TYPE_AVENGER) { // ??????? ?????????? ????? ??????..
		if(nIndex == SKILL_AVENGER_AWAKENING) { // ???? ?????????? ??????????? ???..
			...
			sq_requestBuySkill(SKILL_EXECUTION, flag, count); // 처???? ???? ???????? ????????..
		}
	}
...
function onAttack_PassiveObject(passiveobj, damager, bounding_box, is_stuck)
{
	...
	setMonsterActAppendageEffect_New(passiveobj, damager, bounding_box, is_stuck);
		...
		if(!isGrowTypeAvenger(pChr)) return -1; // 占??占??占?? 占??占??占??占?? 占??占???占??占?占?????..

		if(pChr) { 
			// 占????占??占??占??占??占??占??? 占??占??占???占??占??占??占??占?? 占??占??占???..占???? 占??占??占???? 占???? 占??占??占?? 占??? 占??占??占??占?占????..
			// 24104	`Character/Priest/DevilStrike1.obj`	// 占??占?? - 占??占??占???占??占??占??占??1
			// 24105	`Character/Priest/DevilStrike2.obj`	// 占??占?? - 占??占??占???占??占??占??占??2
			// 24106	`Character/Priest/DevilStrike3.obj`	// 占??占?? - 占??占??占???占??占??占??占??3
			
			if(passiveobj.getPassiveObjectIndex() != 24104 && passiveobj.getPassiveObjectIndex() != 24105 &&
			passiveobj.getPassiveObjectIndex() != 24106)
				procDevilStrikeGauge(pChr, passiveobj.getPassiveObjectIndex()); // 占??占?? state占?? ??占??? 占??占??占?? 占???占?? 占??占???占?占???占??占??占??占?? 占??占??占??占??占????..
		}
	}
...
function sqr_CreatePooledObject(obj, ani_filename, x, y, z, dir)
{
	...
	local ani = obj.sq_createCNRDAnimation(ani_filename); // [create draw only object] ?? ???? ???? ?????? ????? ??????????..
...
function getCurrentModuleDamageRate(obj)
{
	...
	if (skill_level > 0)
	{
		rate += sq_GetIntData(obj, 169, 50) / 100.0 * 1.3;
	}
