...
//	  outPut("dummy/dummy.txt","10");
    ...
    local t = appendage.getVar().get_timer_vector(0);//孃쀥댆?귡릺
...
function onStart_appendage_NewStyleEnergyMonster(appendage)
{
	...
	t.setParameter(100, -1);//?껅빖1竊싨あ?ι뼋???껅빖2竊싮솏?뜻А??
	t.resetInstant(0);//?띸쉰 ?껅빖?ょ윥
	t.setEventOnStart(false);//?틊rue 訝?뗥쭓弱긷룾餓θ㎏??
}
